...
class MessageDisplay:
    ...
    def word_wrap(self, surface, text, font, color=(255, 255, 255)):
        full_rect = font.get_rect(text)
        # only wrap if text doesn't fit
        if full_rect.width <= 0.8 * surface.get_width():
            x, y = (
                0.5 * surface.get_width() - 0.5 * full_rect.width,
    ...
    def render_message(self, screen):
        if self.show:
            self.word_wrap(screen, self.message, self.font)
